...
public class GuiProvider extends GuiContainer
{
    private static ResourceLocation BACKGROUND = new ResourceLocation(WoodenGears.MODID,
            "textures/gui/provider.png");

    private final TileProvider provider;

    public GuiProvider(EntityPlayer player, TileProvider armReservoir)
    {
        super(armReservoir.createContainer(player));

        this.provider = armReservoir;
    }
    ...
    {
        this.fontRenderer.drawString(this.provider.getDisplayName().getUnformattedText(), 8, 6, 4210752);
    }
